...
# Configure logging
logging.basicConfig(filename='flask_app.log', level=logging.DEBUG, format='%(asctime)s - %(message)s')

# Load model
ml_model = load("random_forest_model.joblib")
logging.info("ML model initialized.")
...
def predict_attack():
    ...
    logging.info("Processing prediction.")
    try:
        input_data = {feature: float(request.form.get(feature, 0)) for feature in INPUT_FEATURES}
        input_df = pd.DataFrame([input_data], columns=INPUT_FEATURES)
        prediction = ml_model.predict(input_df)[0]
        logging.info(f"Outcome: {prediction}")
        return render_template('index.html', prediction=f"Outcome: {prediction}")
    except Exception as e:
        logging.error(f"Error in prediction: {e}")
        return render_template('index.html', error=f"Error: Please check input values. {e}")
